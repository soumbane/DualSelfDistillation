...
##############################################################################################################################
## Below is code that worked with 300 epochs with 5 filters [32,64,128,256,320] and hence 3 enc-dec layers - for MMWHS CT/MR datasets
...
class SelfDistilDynUNet(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.training and self.deep_supervision:
            ...
            # For Deep Supervision ONLY (CT3) - currently done for MMWHS CT with nnUnet
            # out = (out_main, out_dec3, out_dec2, out_dec1) 
            
            # For nnUNet with Deep Super, Self-Distill & Self-Distill with Dist Maps
...
##############################################################################################################################
## Below is code that worked with 300 epochs with 6 filters [16,32,64,128,256,320] and hence 4 enc-dec layers
...
#         # for self-distillation
#         if self.super_head is None and self.enc_head is not None and self.e_heads is not None and self.index > 0:
#             self.e_heads[self.index - 1] = self.enc_head(downout)  # type:ignore
#             print(f'Encoder bottleneck input shape at index {self.index} is: {downout.shape}')
#             print(f'Encoder bottleneck output shape at index {self.index} is: {self.e_heads[self.index-1].shape}')
#             # # print(self.e_heads[1].shape)
#             # # print(self.e_heads[1])
#             # print(self.enc_head)
#             raise ValueError
...
#         # for self-distillation
#         if self.super_head is None and self.dec_head is not None and self.d_heads is not None and self.index > 0:
#             self.d_heads[self.index - 1] = self.dec_head(upout)  # type:ignore
#             # print(f'Decoder bottleneck input shape at index {self.index} is: {upout.shape}')
#             # print(f'Decoder bottleneck output shape at index {self.index} is: {self.d_heads[self.index-1].shape}')
#             # print(self.dec_head)
#             # raise ValueError
...
#                 super_distill_flag = False
#                 if index == 0:  # don't associate a distillation head with self.input_block
#                     rest_enc_heads = distil_enc_heads
#                     rest_dec_heads = distil_dec_heads
#                     # print(f'Index: {index}')
#                     # print("Rest Encoder Heads: ")
...
#                 else:
#                     if len(distil_enc_heads) > 0 or len(distil_dec_heads) > 0:  
#                         super_distill_flag = True                  
#                         rest_enc_heads = distil_enc_heads[1:]
#                         rest_dec_heads = distil_dec_heads[1:]
#                         # print(f'Index: {index}')
#                         # print("Rest Encoder Heads: ")
#                         # print(rest_enc_heads)
#                         # print("Rest Decoder Heads: ")
...
#                         e_heads=self.e_heads,
#                         d_heads=self.d_heads,
#                         enc_head=distil_enc_heads[0],
#                         dec_head=distil_dec_heads[0],
#                     )
